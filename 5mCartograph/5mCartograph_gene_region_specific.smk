...
# 5mCartograph_smk_3_summarize_calculate_and_output_gene_region_specific.py was optimized to only use data from regions of interest (genes + 1 kb up- and downstream)
...
# 5mCartograph_smk_4_plot_results_gene_region_specific.py is nearly identical to the original version
# it simply plots an altered y-axis label and plot data slightly different
...
# 5mCartograph_smk_3_summarize_calculate_and_output_gene_region_specific.py -> summarizes data of subsets and calculates data of interest for genes/region of interest only
# 5mCartograph_smk_4_plot_results_gene_region_specific.py -> plots results with matplotlib for genes/region of interest only

# tsv needs to be placed in destination directory
# snakefile needs to be used in 'snakemake' environment 
# Command: snakemake all --cores 10 --snakefile 5mCartograph_gene_region_specific.smk --configfile path/to/config.yaml
...
rule result_summary:
    ...
    output:
        ...
        result_all = "{folder}5mCartograph_out3/5mCartograph_out3_log_and_results_gene_region_specific.txt"        
    shell:
        """
        mkdir -p {output.dir_3}
        python3 {tool_folder_name}5mCartograph_smk_3_summarize_calculate_and_output_gene_region_specific.py {input} {result_folder_name}
        """
...
rule plot:
    input:
        plot_data = "{folder}5mCartograph_out3/5mCartograph_out3_log_and_results_gene_region_specific.txt"
    output:
        png = "{folder}5mCartograph_out3/5mCartograph_plot_results_grouped_gene_set_gry.png",
        log = "{folder}5mCartograph_out3/5mCartograph_plot_results_grouped_gene_set_gry_log.txt"
    params:
        title = expand("{title}", title=plot_title)
    shell:
        """
        python3 {tool_folder_name}5mCartograph_smk_4_plot_results_grouped_gene_set_gry.py {input.plot_data} {params.title} 
        """
...
rule all:
    input:
        ...
        result_all = expand("{folder}5mCartograph_out3/5mCartograph_out3_log_and_results_gene_region_specific.txt", folder=result_folder_name),
        png = expand("{folder}5mCartograph_out3/5mCartograph_plot_results_grouped_gene_set_gry.png", folder=result_folder_name),
        log = expand("{folder}5mCartograph_out3/5mCartograph_plot_results_grouped_gene_set_gry_log.txt", folder=result_folder_name)
