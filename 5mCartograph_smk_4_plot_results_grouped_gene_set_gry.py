...
# this script is designed for the plotting of gene set specific methylation rates
# gry = group related y axis
# for each group (5mC, CG, CHG, CHH) 100% means all cytosines of this group

# NEEDED: gene specific 5mCartograph
# /path/to/5mCartograph/5mCartograph_smk_3_summarize_calculate_and_output_gene_region_specific.py
...
# log file
log_file = folder + '/5mCartograph_plot_results_grouped_gene_set_gry_log.txt'
...
def get_data_of_interest(lines):
    ...
    for idx, line in enumerate(lines):
        # all Cs
        if line.startswith('cytosines in genic regions: ', 0):
            C_p = float(line.split(' ')[-2])
        elif line.startswith('highly unmethylated cytosines in genic regions: ', 0):
            values[0] = float(line.split(' ')[-2])
        elif line.startswith('rather unmethylated cytosines in genic regions: ', 0):
            values[1] = float(line.split(' ')[-2])
        elif line.startswith('50/50-methylated cytosines in genic regions: ', 0):
            values[2] = float(line.split(' ')[-2])
        elif line.startswith('rather methylated cytosines in genic regions: ', 0):
            values[3] = float(line.split(' ')[-2])
        elif line.startswith('highly methylated cytosines in genic regions: ', 0):
            values[4] = float(line.split(' ')[-2])
        # CG
        elif line.startswith('CGs in genic regions: ', 0):
            CG_p = float(line.split(' ')[-2])
        elif line.startswith('highly unmethylated CGs in genic regions: ', 0):
            values[5] = float(line.split(' ')[-2])
        elif line.startswith('rather unmethylated CGs in genic regions: ', 0):
            values[6] = float(line.split(' ')[-2])
        elif line.startswith('50/50-methylated CGs in genic regions: ', 0):
            values[7] = float(line.split(' ')[-2])
        elif line.startswith('rather methylated CGs in genic regions: ', 0):
            values[8] = float(line.split(' ')[-2])
        elif line.startswith('highly methylated CGs in genic regions: ', 0):
            values[9] = float(line.split(' ')[-2])
        # CHG
        elif line.startswith('CHGs in genic regions: ', 0):
            CHG_p = float(line.split(' ')[-2])
        elif line.startswith('highly unmethylated CHGs in genic regions: ', 0):
            values[10] = float(line.split(' ')[-2])
        elif line.startswith('rather unmethylated CHGs in genic regions: ', 0):
            values[11] = float(line.split(' ')[-2])
        elif line.startswith('50/50-methylated CHGs in genic regions: ', 0):
            values[12] = float(line.split(' ')[-2])
        elif line.startswith('rather methylated CHGs in genic regions: ', 0):
            values[13] = float(line.split(' ')[-2])
        elif line.startswith('highly methylated CHGs in genic regions: ', 0):
            values[14] = float(line.split(' ')[-2])    
        # CHH
        elif line.startswith('CHHs in genic regions: ', 0):
            CHH_p = float(line.split(' ')[-2])
        elif line.startswith('highly unmethylated CHHs in genic regions: ', 0):
            values[15] = float(line.split(' ')[-2])
        elif line.startswith('rather unmethylated CHHs in genic regions: ', 0):
            values[16] = float(line.split(' ')[-2])
        elif line.startswith('50/50-methylated CHHs in genic regions: ', 0):
            values[17] = float(line.split(' ')[-2])
        elif line.startswith('rather methylated CHHs in genic regions: ', 0):
            values[18] = float(line.split(' ')[-2])
        elif line.startswith('highly methylated CHHs in genic regions: ', 0):
...
with open(log_file, 'w') as fileOut:
    ...
    fileOut.write('5mCartograph - gene set specific 5mC, CG, CHg and CHH methylation probability distribution figure (grouped)\n')
...
plt.savefig(folder+'/5mCartograph_plot_results_grouped_gene_set_gry.png', bbox_inches='tight')
plt.show()
